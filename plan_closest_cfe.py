...
This version is identical to the original, EXCEPT:
    - when saving plans, it keeps ONLY the single candidate value per feature
      that is closest to the original instance value (still as a 1-element list).
...
Format:
    {
      "<test_idx>": {
        "<feature_name>": [single_closest_value],
        ...
      },
      ...
    }
...
def build_plans_for_project(
    ...
    For a single project:
        ...
        - write plans_all.json under ./plans/{project}/{model_type}/{explainer_name},
          keeping only the single closest value per feature.
    """
    ...
    plans_dir = Path(f"./plans_closest/{project_name}/{model_type}/{explainer_name}")
    plans_dir.mkdir(parents=True, exist_ok=True)
    plans_path = plans_dir / "plans_all.json"

    all_plans_full: Dict[int, Dict[str, List[float]]] = {}

    # ------------------------------ CfExplainer branch ------------------------------
    if explainer_name == "CfExplainer":
        for test_idx in tqdm(
            tps.index,
            desc=f"{project_name} (Cf→plans-closest)",
            leave=False,
            disable=not verbose,
        ):
            ...
            if perturb_features:
                all_plans_full[int(test_idx)] = perturb_features

    # ------------------------------ PyExplainer branch ------------------------------
    elif explainer_name == "PyExplainer":
        for test_idx in tqdm(
            tps.index,
            desc=f"{project_name} (Py→plans-closest)",
            leave=False,
            disable=not verbose,
        ):
            test_instance = test.loc[test_idx]
    ...
    # ------------------------------ Compress to single closest per feature ------------------------------
    all_plans_single: Dict[int, Dict[str, List[float]]] = {}
    for idx, feat_dict in all_plans_full.items():
        single_feat_dict: Dict[str, List[float]] = {}
        for feat, vals in feat_dict.items():
            if not vals:
                continue
            # vals are already sorted by distance → take first, but keep as [value]
            v = vals[0]
            single_feat_dict[feat] = [float(v)]
        if single_feat_dict:
            all_plans_single[idx] = single_feat_dict

    # Save all plans for this project
    if all_plans_single:
        with open(plans_path, "w") as f:
            json.dump(all_plans_single, f, indent=4, default=convert_int64)
        print(f"[INFO] Saved plans (closest only) for {project_name} ({explainer_name}) -> {plans_path}")
    else:
...
def main():
    parser = ArgumentParser(
        description="Convert CfExplainer / PyExplainer outputs into planner-style plans_all.json "
                    "keeping only the closest value per feature.",
    )
    parser.add_argument(
        "--model_type",
        type=str,
