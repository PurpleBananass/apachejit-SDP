...
def build_history_distribution(ds, project_list):
    """
    Build global history distribution across project_list.

    JIT assumption:
      - Each row in TRAIN is already a "change" snapshot (commit-level metrics).
      - So we use the raw TRAIN feature vectors directly (no consecutive deltas).

    NaN/inf handling:
      - We do NOT drop rows; we only filter/repair columns.
      - Drop columns that are completely NaN, have too few finite values,
        or are (almost) constant.
      - For remaining columns, impute NaN/inf with the column median
        (or 0.0 if the median cannot be computed).
    """
    history_list = []
    ...
    for project in project_list:
        ...
        if len(train) == 0 or not feat_cols:
            print(f"[RQ3] {project}: skipped (train rows={len(train)}, feat_cols={len(feat_cols)})")
            continue

        feats = train[feat_cols].copy()
        print(f"[RQ3] {project}: train rows={len(feats)}, features={len(feat_cols)}")
        history_list.append(feats)

    if not history_list:
        raise RuntimeError("No historical data could be built from train snapshots.")

    total_hist = pd.concat(history_list, axis=0, ignore_index=True)
    print(f"[RQ3] History matrix before cleaning: rows={total_hist.shape[0]}, "
          f"cols={total_hist.shape[1]}")

    # 1) Replace inf/-inf with NaN
    total_hist = total_hist.replace([np.inf, -np.inf], np.nan)

    # 2) Drop columns that are completely NaN
    total_hist = total_hist.dropna(axis=1, how="all")
    if total_hist.empty:
        raise RuntimeError("All history columns were NaN/inf in every row.")

    # 3) Drop columns that have too few finite values OR are (almost) constant
    keep_cols = []
    for col in total_hist.columns:
        vals = total_hist[col].to_numpy(dtype=float)
        finite_mask = np.isfinite(vals)
        n_finite = finite_mask.sum()
        if n_finite < 5:     # threshold can be adjusted
            continue

        var = np.var(vals[finite_mask])
        if var <= 1e-12:
            continue
    ...
    if total_hist.empty:
        raise RuntimeError(
            "All features were dropped as low-support or constant in history."
        )

    print(f"[RQ3] History matrix after column filtering: rows={total_hist.shape[0]}, "
          f"cols={total_hist.shape[1]}")

    # 4) Impute remaining NaNs per column (median; fallback to 0.0)
    for col in total_hist.columns:
        vals = total_hist[col].to_numpy(dtype=float)
        ...
        if finite_vals.size == 0:
            ...
            fill_val = np.median(finite_vals)
        total_hist[col] = np.where(np.isfinite(vals), vals, fill_val)

    # 5) Build X and covariance with a small ridge
    ...
    feat_cols_md = list(total_hist.columns)
    print(f"[RQ3] Final history matrix: rows={X.shape[0]}, cols={X.shape[1]}")
    return total_hist, feat_cols_md, mean_vec, inv_cov
...
def compute_mahalanobis_scores(
    ...
    the *flipped commit feature vector* to the global history mean.

    History and flips now live in the same space:
      - history: raw TRAIN snapshots
      - flips: raw flipped TEST snapshots (with edits applied)
    """
    scores = []
    ...
    for project in project_list:
        ...
        for _, row in flips.iterrows():
            ...
            # apply edited values from flip row
            cand = orig.copy()
            ...
            if np.allclose(cand.values, orig.values, rtol=1e-7, atol=1e-7):
                continue

            # embed cand into the history feature space
            x_full = np.zeros(len(feat_cols_md), dtype=float)
            for f in feat_cols_proj:
                if f in feat_index:
                    x_full[feat_index[f]] = float(cand[f])

            # safety: if everything is zero (no overlapping features), skip
            if np.allclose(x_full, 0.0, rtol=1e-7, atol=1e-7):
                continue

            d = float(mahalanobis(x_full, mean_vec, inv_cov))
            scores.append(
...
def plot_rq3_on_axis(ax, dist_data_dict, model_abbr):
    ...
    # --- axes styling ---
    ax.set_xlim(0.0, 10.0)          # cut at 20
    ax.set_ylim(bottom=0)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
...
def plot_rq3_vertical(all_dist_dicts, save_path):
    ...
    fig, axes = plt.subplots(
        nrows=n_models,
        ncols=1,
        figsize=(6, 2.4 * n_models),   # width from 12 -> 6
        sharex=True,
    )

    # when n_models == 1, axes is a single Axes, not a list
    if n_models == 1:
        axes = [axes]
...
if __name__ == "__main__":
    ...
    print(
        f"[RQ3] Actual History distances: N={len(hist_vals)}, "
        f"min={hist_vals.min():.3f}, max={hist_vals.max():.3f}, "
        f"mean={hist_vals.mean():.3f}"
    )
