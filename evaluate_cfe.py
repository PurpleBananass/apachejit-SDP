...
def implications(project, explainer, model_type, compare_with_cf: bool = False):
    ...
    def cf_first_rows(cf_df: pd.DataFrame) -> pd.DataFrame:
        return cf_df.sort_values(
            ["test_idx"] + (["candidate_id"] if "candidate_id" in cf_df.columns else []),
            kind="stable"
        ).groupby("test_idx", as_index=False).head(1)
...
# ----------------------------- RQ3 feasibility (legacy) -----------------------------
# (unchanged from your version; CfExplainer will be treated like other plan-based explainers)
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        ...
        help=('Explainers spaced/comma (e.g., "CF LIME"), or "all": '
              'LIME LIME-HPO PyExplainer CfExplainer CF')  # <<< help text updated
    )
    ...
    if args.explainer.strip().lower() == "all":
        explainers = ["LIME", "LIME-HPO", "PyExplainer", "CfExplainer", "CF"]  # <<< CfExplainer added
    else:
